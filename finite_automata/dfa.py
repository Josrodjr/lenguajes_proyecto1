...
def fill_transitions(tree):
    ...
    for transition in transitions:
        if transition[1] != 0:
            if chr(transition[1]) not in trans:
                trans.append(chr(transition[1]))
                ...
                if len(local_state_trans[reachable_state]['transitions'][new_state_index]) is 0:
                    ...
                    if found_state != 0:
                        ...
                        for transition in loctrans:
                            mov_states = mov(tree.automata, local_state_trans[reachable_state]['transitions'][new_state_index], ord(transition))
...
def generate_dfa(tree):
    ...
    loc_transitions = []
    for transition in transitions:
        mov_states = mov(tree.automata, first_transitions, ord(transition))
        loc_transitions.append(mov_states)
    # perform eclosure on the set to generate new node in tree
    e_transitions = []
    for transition in loc_transitions:
